...
Рефлексия к задаче #12:
	Сложность временная остается такой же, как и при сортировке отдельного списка, ее ускорить теоретически нельзя,
	можно лишь повлиять на оптимизацию памяти.

	Результирующий список не нуждается в сортировке, так как он уже был отсортирован, потому что при его создании
	были использованы 2 отсортированных списка.
*/

func CombineLists(l1, l2 LinkedList2) LinkedList2 {
	if l1.Count() != l2.Count() || l1.head == nil {
		return LinkedList2{}
	}
